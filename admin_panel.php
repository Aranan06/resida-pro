...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
  ...
  // Site Ekle
    ...
    // Paket Ekle / Düzenle / Sil
        ...
        if($name&&$priceM){ $pdo->prepare("INSERT INTO subscription_plans (name,max_residents,price_monthly,price_yearly,features) VALUES (?,?,?,?,?)")->execute([$name,$maxR,$priceM,$priceY?:null,json_encode($feat,JSON_UNESCAPED_UNICODE)]); $success='Paket eklendi.'; } else $error='Paket adı ve aylık fiyat zorunlu.';
        ...
        $feat=array_values(array_filter(array_map('trim',preg_split('/\r\n|\n|\r/',$_POST['features']??''))));
        if($id&&$name){ $pdo->prepare("UPDATE subscription_plans SET name=?, max_residents=?, price_monthly=?, price_yearly=?, features=? WHERE id=?")->execute([$name,$maxR,$priceM,$priceY?:null,json_encode($feat,JSON_UNESCAPED_UNICODE),$id]); $success='Paket güncellendi.'; }

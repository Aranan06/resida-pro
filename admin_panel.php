...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
  ...
  // Site Ekle
    ...
    // Paket Ekle / Düzenle / Sil
        ...
        $feat=array_values(array_filter(array_map('trim',preg_split('/\r\n|\n|\r/',$_POST['features']??'')))); $tag=trim($_POST['tagline']??''); $isF=!empty($_POST['is_featured'])?1:0;
        if($name&&$priceM){ $pdo->prepare("INSERT INTO subscription_plans (name,max_residents,price_monthly,price_yearly,features,tagline,is_featured) VALUES (?,?,?,?,?,?,?)")->execute([$name,$maxR,$priceM,$priceY?:null,json_encode($feat,JSON_UNESCAPED_UNICODE),$tag,$isF]); if($isF) $pdo->prepare("UPDATE subscription_plans SET is_featured=0 WHERE id!=?")->execute([$pdo->lastInsertId()]); $success='Paket eklendi.'; } else $error='Paket adı ve aylık fiyat zorunlu.';
        ...
        $feat=array_values(array_filter(array_map('trim',preg_split('/\r\n|\n|\r/',$_POST['features']??'')))); $tag=trim($_POST['tagline']??''); $isF=!empty($_POST['is_featured'])?1:0;
        if($id&&$name){ $pdo->prepare("UPDATE subscription_plans SET name=?, max_residents=?, price_monthly=?, price_yearly=?, features=?, tagline=?, is_featured=? WHERE id=?")->execute([$name,$maxR,$priceM,$priceY?:null,json_encode($feat,JSON_UNESCAPED_UNICODE),$tag,$isF,$id]); if($isF) $pdo->prepare("UPDATE subscription_plans SET is_featured=0 WHERE id!=?")->execute([$id]); $success='Paket güncellendi.'; }
...
<div class="modal-body">
    ...
    <div class="row g-4">
      ...
      <div class="col-md-4">
        <div class="card h-100">
          <div class="card-body">
            <h5 class="fw-700"><?= htmlspecialchars($pl['name']) ?> <?php if(!empty($pl['is_featured'])): ?><span class="badge bg-warning text-dark">Öne Çıkan</span><?php endif; ?></h5>
            <?php if(!empty($pl['tagline'])): ?><div class="small text-primary fw-700"><?= htmlspecialchars($pl['tagline']) ?></div><?php endif; ?>
    ...
    $landingFields=[
      ...
      'Ekranlar ve Fiyatlar'=>['screens_title'=>'Ekranlar başlığı','screens_subtitle'=>'Ekranlar alt yazı|textarea','screens_side_title'=>'Telefon kutusu başlığı','screens_side_text'=>'Telefon kutusu yazı|textarea','pricing_title'=>'Fiyat başlığı','pricing_subtitle'=>'Fiyat alt yazı|textarea','pricing_note'=>'Fiyat alt notu','pricing_btn'=>'Fiyat kartı butonu'],
